    ...
    def step6(self):
        ...
        for file in graphs_sorted:
            ...
            rfile.write('# load table\n'+
                        ...
                        'library(ggplot2)\n'+
                        'library(methods)\n'+
                        'p.afd <- ggplot('+str(self.c1_c2)+', data=test, binwidth=0.001) + geom_vline(xintercept='+str(self.out_afd_up)+', color="grey30", linetype="dashed") + geom_vline(xintercept='+str(self.out_afd_low)+', color="grey30", linetype="dotted") + theme_bw()\n'+
                        'p.dxy <- ggplot(Dxy, data=test, binwidth=0.001, xlim=c(0, (max(test$Dxy)+0.01))) + geom_vline(xintercept='+str(self.out_dxy)+', color="grey30", linetype="dashed") + theme_bw()\n'+
                        'p.fst <- ggplot(Fst, data=test, binwidth=0.001, xlim=c(0, (max(test$Fst)+0.01))) + geom_vline(xintercept='+str(self.out_fst)+', color="grey30", linetype=2) + theme_bw()\n'+
                        'p.dd <- ggplot(DD, data=test, binwidth=0.001) + geom_vline(xintercept='+str(self.out_dd)+', color="grey30", linetype=2) + theme_bw()\n'+
                        'p.length <- ggplot(length, data=test, binwidth=100, xlim=c(0,26560)) + theme_bw()\n\n'+
                        '# export as pdf\n'+
