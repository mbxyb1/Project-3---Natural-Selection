...
parser.add_argument('-cut', type=float, metavar='cutoff_ratio', default='0.1', help='Top percentile, defines outliers [0.1]')
...
if args.out is 'na':
    ...
    for file in suball:
        if file.endswith(args.out+'_'+str(int(args.ovlp))+'ol'+args.suf+'.txt'):
            genelist.append(file)
genelist.sort()
...
for file in genelist:
    ...
    for line in inputfile:
        ...
        for i in range(len(allsites)):
            ...
            rfile = open(outputdir+ttline+basename+args.suf+'.r', 'w')
			...
			'   geom_line(color=I("green")) +\n'+
                        '   geom_hline(yintercept=descr[which(descr$stat=="Cutoff"),3], color="green", linetype=2) +\n'+
                        '   layout\n'+
                        ...
                        '   geom_hline(yintercept=descr[which(descr$stat=="Cutoff"),4], color="blue", linetype=2) +\n'+
                        '   layout\n'+
                        ...
                        '   geom_hline(yintercept=descr[which(descr$stat=="Cutoff"),5], color="goldenrod", linetype=2) +\n'+
                        '   layout\n'+
                        ...
                        '   geom_hline(yintercept=descr[which(descr$stat=="Cutoff"), 2], color="red", linetype=2) +\n'+
                        '   geom_hline(yintercept=descr[which(descr$stat=="Cutoff2"), 2], color="red", linetype=2) +\n'+
                        '   layout\n'+
            ...
            cmd = ('Rscript '+outputdir+ttline+basename+args.suf+'.r')
